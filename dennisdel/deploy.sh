...
BUCKET_NAME=dennisdel.com-cdn

# Make sure we have the latest theme.
cd themes
git clone https://github.com/dend/cocoa-hugo-theme
cd ..
...
aws s3 sync --cache-control "max-age=31536000" --acl "public-read" --sse "AES256"  public/ s3://$BUCKET_NAME
